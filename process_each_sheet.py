...
try:
    ...
    for sheet in wb.sheets:
        ...
        # 提取航司代码 - 从工作表名中提取
        import re
        airline_match = re.match(r'^([A-Z0-9]{2,3})', sheet.name)
        if airline_match:
            standardized['airline'] = airline_match.group(1)
        else:
            # 特殊处理一些工作表名
            if '福' in sheet.name:
                standardized['airline'] = 'FU'
            elif '北' in sheet.name:
                standardized['airline'] = 'GX'
            elif '乌' in sheet.name:
                standardized['airline'] = 'UQ'
            elif '长' in sheet.name:
                standardized['airline'] = '9H'
            elif '华' in sheet.name:
                standardized['airline'] = 'G5'
            elif '奥' in sheet.name:
                standardized['airline'] = 'BK'
            elif '幸' in sheet.name:
                standardized['airline'] = 'JR'
            elif '天' in sheet.name:
                standardized['airline'] = 'GS'
            elif '青' in sheet.name:
                standardized['airline'] = 'QW'
            elif '河' in sheet.name:
                standardized['airline'] = 'NS'
            elif '江' in sheet.name:
                standardized['airline'] = 'RY'
            elif '多' in sheet.name:
                standardized['airline'] = 'GY'
            elif '东' in sheet.name:
                standardized['airline'] = 'DZ'
            elif '产' in sheet.name:
                standardized['airline'] = 'ZZ'  # 汇总表
            else:
                standardized['airline'] = 'XX'  # 未知
        ...
        print(f"  {sheet.name}: {len(standardized)} 条")
